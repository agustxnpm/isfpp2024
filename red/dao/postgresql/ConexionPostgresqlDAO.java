...
public class ConexionPostgresqlDAO implements ConexionDAO {
	...
	public Conexion buscarPorCodigo(String equipo1Codigo, String equipo2Codigo) {
		...
		try {
			...
			rs = pstm.executeQuery();
			if (rs.next()) {
				Conexion conexion = new Conexion(equipos.get(rs.getString("equipo1")),
						equipos.get(rs.getString("equipo2")), tipoCable.get(rs.getString("tipo_cable")),
						tipoPuerto.get(rs.getString("tipo_puerto1")), tipoPuerto.get(rs.getString("tipo_puerto2")));
				return conexion;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
